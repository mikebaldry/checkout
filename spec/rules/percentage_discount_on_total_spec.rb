RSpec.describe Rules::PercentageDiscountOnTotal do
  ...
  let(:value_15) { BasketProduct.new(product: product_a, quantity: 3) }
  let(:value_2_50) { BasketProduct.new(product: product_b, quantity: 1) }
  let(:value_5) { BasketProduct.new(product: product_b, quantity: 2) }
  let(:value_7_50) { BasketProduct.new(product: product_b, quantity: 3) }
  ...
  describe '#apply' do
    context 'the total is less than the minimum total' do
      let(:basket) { [value_15, value_2_50] }
    ...
    context 'the total is exactly than the minimum total' do
      ...
      it 'applies the discount to all items' do
        ...
        expect(subject.apply(basket)).to eq([discounted_15, discounted_5])
      end
    end

    context 'the total is above the minimum total' do
      let(:basket) { [value_15, value_7_50] }

      it 'applies the discount to all items' do
        discounted_15 = value_15.discounted_by(0.5) # 3 * 5, 10% of 5 = 0.5
        discounted_7_50 = value_7_50.discounted_by(0.25) # 3 * 2.5, 10% of 2.5 = 0.25

        expect(subject.apply(basket)).to eq([discounted_15, discounted_7_50])
      end
    end
  end
end
